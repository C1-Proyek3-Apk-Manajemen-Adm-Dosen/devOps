...
const dokumenRoute = window.dokumenRoute;
...
document.getElementById('addAccessBtn')?.addEventListener('click', async function() {
    ...
    for (const cb of checked) {
        ...
        if (document.querySelector(`[data-user-id="${userId}"]`)) {
            showAlert(`${userName || 'User'} sudah memiliki akses`, 'warning');
            cb.checked = false;
            continue;
        }
        ...
        try {
            ...
            if (data.success) {
                addUserToList(userId, userName || 'User', userEmail || '', userRole || 'User', permission);
                cb.checked = false;
                showAlert(data.message ?? `Akses ditambahkan untuk ${userName}`, 'success');
            } else {
                showAlert(data.message ?? `Gagal menambahkan akses untuk ${userName}`, 'error');
            }
        } catch (err) {
            console.error('Error adding access for user', userId, err);
            showAlert(`Gagal menambahkan akses untuk ${userName}`, 'error');
        }
    }
...
function addUserToList(userId, userName, userEmail, role, permission) {
    ...
    const initials = safeName.substring(0, 2).toUpperCase() || 'U';
    ...
    const escapedNameForOnclick = safeName.replace(/'/g, "\\'");
    ...
    const html = `
        <div data-user-id="${userId}" class="flex items-center justify-between bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 transition-all duration-200 group animate-slideIn">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-semibold text-sm text-gray-800">${escapeHtml(safeName)}</p>
                    <p class="text-xs text-gray-500">${escapeHtml(safeEmail)} (${escapeHtml((role || 'User').charAt(0).toUpperCase() + (role || 'User').slice(1))})</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                ...
                <button type="button" onclick="removeAccess(${userId}, '${escapedNameForOnclick}')" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100" title="Hapus Akses">
...
window.removeAccess = function(userId, userName) {
    if (!confirm(`Hapus akses untuk ${userName}?`)) return;
...
function showAlert(message, type) {
    ...
    alert.className = `fixed top-4 right-4 ${colors[type]} text-white px-6 py-3 rounded-xl shadow-lg flexitems-center gap-3 animate-slideInRight z-[60]`;
    alert.innerHTML = `
